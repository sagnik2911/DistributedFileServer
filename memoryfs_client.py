...
class DiskBlocks():
    def __init__(self, server_urls):
        self.block_server = {}
        self.block_map = {}
        self.number_of_servers = len(server_urls)
        for i in range(self.number_of_servers):
            self.block_server[i] = xmlrpc.client.ServerProxy(server_urls[i], use_builtin_types=True)
        self.InitializeMap()

    ## InitializeMap: creating predefined mapping between
    ## Blocks are padded with zeroes up to BLOCK_SIZE

    def InitializeMap(self):

        number_of_dataservers = self.number_of_servers - 1
        number_of_blocks_per_server = TOTAL_NUM_BLOCKS / self.number_of_servers if TOTAL_NUM_BLOCKS % self.number_of_servers else TOTAL_NUM_BLOCKS / self.number_of_servers + 1
        logging.info(
            'Initializing Blocks: number of servers ' + str(self.number_of_servers) + ' blocks per server ' + str(
                number_of_blocks_per_server))
        parityserver = number_of_dataservers

        for block_number_begin in range(0, TOTAL_NUM_BLOCKS, number_of_dataservers):
            if parityserver == -1:
                parityserver = number_of_dataservers

            for block_number in range(block_number_begin, block_number_begin + number_of_dataservers):
                server_number = block_number % number_of_dataservers
                block_number_on_server = block_number // number_of_dataservers

                if server_number >= parityserver:
                    server_number = server_number + 1

                map_tuple = {"id": server_number, "block_number": block_number_on_server, "parity_server": parityserver}
                logging.info(
                    'Initializing Blocks:  block_number ' + str(block_number) + ' server number ' + str(server_number) +
                    ' block no in server ' + str(block_number_on_server) + ' parity server ' + str(parityserver))
                self.block_map[block_number] = map_tuple
            parityserver = parityserver - 1
    ...
    def Put(self, block_number, block_data):
        ...
        if block_number in range(0, TOTAL_NUM_BLOCKS):
            ...
            server_number = self.block_map[block_number]["id"]
            block_number_on_server = self.block_map[block_number]["block_number"]
            parity_server = self.block_map[block_number]["parity_server"]

            logging.info('Putting Block:  block_number ' + str(block_number) + ' server number ' + str(server_number) +
                         ' block no in server ' + str(block_number_on_server))
            logging.info('Putting Parity Block:  block_number ' + str(block_number) + ' parity server number ' + str(
              parity_server) +
                         ' block no in server ' + str(block_number_on_server))

            xorddata = bytearray(BLOCK_SIZE)

            olderdata = self.block_server[server_number].Get(block_number_on_server)
            if olderdata == -1:
                olderdata = self.recover_Get(block_number_on_server, server_number)
            else:
                olderdata = bytearray(olderdata)

            paritydata = self.block_server[parity_server].Get(block_number_on_server)
            if paritydata == -1:
                paritydata = self.recover_Get(block_number_on_server, parity_server)
            else :
                paritydata = bytearray(paritydata)

            # logging.info('Parity Calculation:  Previous Data ' + str(olderdata) + ' Parity Data ' + str(paritydata) +
            #            ' current Data ' + str(putdata))

            for i in range(BLOCK_SIZE):
                xorddata[i] = putdata[i] ^ olderdata[i] ^ paritydata[i]

            retoriginal = self.block_server[server_number].Put(block_number_on_server, putdata)
            if retoriginal == -1:
                logging.error('Put: Server returns error')

            returnparity = self.block_server[parity_server].Put(block_number_on_server, xorddata)
            if returnparity == -1:
                logging.error('Put: Parity Server returns error')

            # both server is faulty, our code can't handle redundancy
            if retoriginal == -1 and returnparity == -1:
                quit()
    ...
    def Get(self, block_number):
        ...
        if block_number in range(0, TOTAL_NUM_BLOCKS):
            ...
            try:
                server_number = self.block_map[block_number]["id"]
                block_number_on_server = self.block_map[block_number]["block_number"]
                logging.info('Getting Blocks:  block_number ' + str(block_number) + ' server number ' + str(server_number) +
                         ' block no in server ' + str(block_number_on_server))
                data = self.block_server[server_number].Get(block_number_on_server)

                if data != -1:
                    #logging.debug ('Get: data type: ' + str(type(data)) + ', data: ' + str(data))
                    return bytearray(data)

                logging.error('Get: Corrupted Block ' + str(block_number))
                return self.recover_Get(block_number_on_server, server_number)
            except ConnectionRefusedError:
                logging.info('Get Failed for server: ' + str(server_number))
                return self.recover_Get(block_number_on_server, server_number)
    ...
    # begin recovery mode
    def recover_Get(self, block_number_on_server, server_number):

        xorddata = bytearray(BLOCK_SIZE)
        for server_iterator in range(self.number_of_servers):
            if server_iterator == server_number:
                continue
            existdata = bytearray(self.block_server[server_iterator].Get(block_number_on_server))
            for i in range(BLOCK_SIZE):
                xorddata[i] = xorddata[i] ^ existdata[i]

        return bytearray(xorddata)
    ...
    def RSM(self, block_number):
        ...
        if block_number in range(0, TOTAL_NUM_BLOCKS):
            server_number = self.block_map[block_number]["id"]
            block_number_on_server = self.block_map[block_number]["block_number"]
            data = self.block_server[server_number].RSM(block_number_on_server)
            return bytearray(data)
